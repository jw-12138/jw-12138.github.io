<template>
  <div class="lg:block hidden mt-8 relative left-[-.5rem]">
    <div class="px-2 py-1 rounded-xl hover:bg-black/10 dark:hover:bg-white/10 transition-all w-8 h-8 flex justify-center items-center mb-1 cursor-pointer opacity-70 hover:opacity-100"
         @click="goBackToTop"
    >
    ...
    <div
      ...
      :title="heading.text"
      class="overflow-hidden text-ellipsis whitespace-nowrap text-xs max-w-full cursor-pointer hover:bg-black/10 dark:hover:bg-white/10 pt-[.35rem] pb-[.31rem] px-2 transition-all"
      v-for="(heading, index) in data"
      :class="{
        'rounded-t-xl hover:rounded-b-0': (!data[index - 1]) || (data[index - 1] && !slugPositionMap[data[index - 1].slug]?.isInView),
        'rounded-b-xl hover:rounded-t-0': (!data[index + 1]) || (data[index + 1] && !slugPositionMap[data[index + 1].slug]?.isInView),
        'text-black dark:text-white opacity-1 bg-black/10 dark:bg-white/10': slugPositionMap[heading.slug]?.isInView,
        'opacity-70 hover:rounded-xl': !slugPositionMap[heading.slug]?.isInView
      }"
      :style="{
