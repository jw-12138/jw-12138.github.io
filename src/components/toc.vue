<template>
  <div class="lg:block hidden mt-8 relative left-[-.5rem]">
    <div class="px-2 py-1 rounded-xl hover:bg-black/15 dark:hover:bg-white/15 transition-all w-8 h-8 flex justify-center items-center mb-1 cursor-pointer opacity-70 hover:opacity-100"
         @click="goBackToTop"
    >
    ...
    <div
      ...
      :title="heading.text"
      class="text-xs max-w-full cursor-pointer hover:bg-black/15 dark:hover:bg-white/15 px-2 transition-all duration-500"
      ...
      :class="{
        ...
        'opacity-70 hover:rounded-xl pt-[.25rem] pb-[.21rem]': !slugPositionMap[heading.slug]?.isInView
      }"
      :style="{
      paddingLeft: (heading.depth - maxDepth + 1) * .5 + 'rem'
    }">
      <div class="overflow-hidden text-ellipsis whitespace-nowrap py-[.1rem]">
        {{ heading.text }}
      </div>
    </div>
  </div>
